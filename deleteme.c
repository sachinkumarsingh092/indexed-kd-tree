  ...
  while(1)
    {
      ...
      printf("\tnode_store=%u node_mid=%u\n", storeIndex->index, mid->index);
                    ...
                    size_t current_axis, gal_data_t **coords, size_t dim)
{
  ...
  double *carr=coords[current_axis]->array;
	printf("node_current=%u, node_median=%u (input[c]=%f input[m]=%f)\n", 
         current_node->index, median_node->index,
         carr[current_node->index],
         carr[median_node->index]);
  ...
  if(median_node)
    {
      ...
      printf("Setting left subtree (in dim %zu, for med=%u)\n", current_axis, median_node->index);
      median_node->left  = kdtree_fill_subtrees(current_node,
                                     						median_node-current_node,
                                     						current_axis, coords, dim);
      // printf("left[%u] =%f\n", median_node->index, carr[median_node->left->index]);
      printf("Setting right subtree (in dim %zu, for med=%u)\n", current_axis, median_node->index);
      median_node->right = kdtree_fill_subtrees(median_node + 1,
                                     						current_node + remaining - (median_node + 1),
                                     						current_axis, coords, dim);
      // printf("right[%u] =%f\n\n",median_node->index, carr[median_node->right->index]);
    }
